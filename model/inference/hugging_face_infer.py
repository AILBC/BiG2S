...
from .hugging_face_beamsearch import BeamSearchScorer
...
class Beam_Generate(BeamGenerateBase):
    ...
    def generate(
        ...
        next_token_logits = self._sample_process(next_token_logits, min_keep=self.beam_size) if self.all_token.size(-1) == 1\
        ...
        for group_id in range(self.beam_group):
            ...
            if self.top_k > 0 or self.top_p > 0.:
                next_tokens = torch.multinomial(group_probs, num_samples=2 * self.beam_per_group)
